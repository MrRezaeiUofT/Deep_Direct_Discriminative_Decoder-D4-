...
class Combiner(nn.Module):
    ...
    def forward(self, z_t_1, h_rnn, mode='joint'):
        """
        ...
        if mode == 'joint':
            ...
            h_combined = 0.5 *self.tanh(self.lin_z_to_hidden(z_t_1))
...
def get_result_dkf(dkf, sample_traject_length,sample_size,input,  T, mode):
    all_samples = []
    for i_sample in range(sample_traject_length):
        # define initial hidden state
        h_0_contig = dkf.h_0.expand(1, sample_size, dkf.rnn.hidden_size).contiguous()

        # define num_latent samples
        num_latent_samples = sample_size

        # Expand z_prev to have dimensions (num_latent_samples, latent_size)
        z_prev = torch.distributions.Normal(dkf.z_0, dkf.sigma).sample((1,))[0].expand(num_latent_samples,
                                                                                       dkf.z_0.size(0))
        ...
        # Train Predictions
        rnn_output, rnn_hidden_state = dkf.rnn(input.float(), h_0_contig)
        ...
        for t in range(T):
            ...
            z_loc, z_scale = dkf.combiner(z_prev, rnn_output[:, t, :], mode)
            Z_t = z_loc.mean(dim=0).cpu().detach()
            if t != T - 1:
                z_prev = Z_t
            z_samples.append(Z_t.numpy())

        all_samples.append(np.array(z_samples))
    return all_samples
